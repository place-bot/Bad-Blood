...
2. Verify the Pi's actual model, `aarch64` architecture, Raspberry Pi OS, Python version, package availability, Wi-Fi/hotspot connectivity, and free space. Create a project-only venv on the Pi, install compatible **ARM64 CPU** NumPy, SciPy and PyTorch packages, and record their versions. Do not install the training-only dependencies.
