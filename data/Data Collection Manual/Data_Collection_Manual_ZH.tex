...
\section*{7. V1 完整误差指标}
V1：高分辨率双通道残差一维 CNN，使用标准化目标的 MSE 训练。以下指标来自已选定模型的保存预测，权重未变。
\subsection*{官方跨人测试集：144 人／57,600 段}
...
Bias & -1.92 & -1.64\\
误差 SD & 15.89 & 10.66\\
误差不超过 5 mmHg（\%） & 26.06 & 37.24\\
误差不超过 10 mmHg（\%） & 48.80 & 66.11\\
误差不超过 15 mmHg（\%） & 67.55 & 84.62\\
参与者平均 MAE & 12.46 & 8.46\\
参与者 MAE：中位数 & 11.02 & 7.32\\
参与者 MAE：第90百分位 & 20.80 & 14.35\\
\bottomrule\end{tabular}\par\smallskip
}
\subsection*{AAMI 命名子集：116 人／666 段}
{\small\renewcommand{\arraystretch}{1.05}
...
Bias & -12.74 & -9.31\\
误差 SD & 22.72 & 13.93\\
误差不超过 5 mmHg（\%） & 19.22 & 24.77\\
误差不超过 10 mmHg（\%） & 36.19 & 45.20\\
误差不超过 15 mmHg（\%） & 53.00 & 67.27\\
参与者平均 MAE & 19.58 & 13.10\\
参与者 MAE：中位数 & 15.95 & 11.46\\
参与者 MAE：第90百分位 & 36.76 & 23.72\\
\bottomrule\end{tabular}\par\smallskip
}
误差为预测值减参考值；SD 使用 N-1。除百分比外单位均为 mmHg，区间包含边界。参与者平均 MAE 对每人等权。这些是公共数据评估，不是袖带设备临床验证。完整验证集、排除前期接触者后的指标及逐参与者 MAE 均在 V1 的 reports 文件夹。
\end{document}